...
*/
#include<stdio.h>
#include<stdlib.h>  //khai báo thư viện này để dùng calloc, malloc, free, gets, puts
...
void pointerOfPrimitiveType(){
	...
	// ptr= &value;
//	ptr = malloc(sizeof(int));								//Hàm malloc và calloc sẽ trả về con trỏ void lưu địa chỉ của ô nhớ đầu tiên trong vùng nhớ được cấp phát
	ptr = calloc(1,sizeof(int));							// vì là trả về 1 địa chỉ, nên ta dùng ptr để lưu trữ giá trị trả về của 2 hàm này
	*ptr = 100;
	printf("%d", *ptr);
...
void structurePointerPointToStructPointerMembers(){
	...
	//2 DÒNG TRÊN CÓ THỂ VIẾT GỌN THÀNH: Person *p = calloc(1, sizeof(Person));
