...
public:
	...
	void setID(char*);          // các hàm dùng để set giá trị mong muốn cho các thành phần private
	void setName(char*);
	void setAddress(char*);
	void setAge(int);
	void setMark(float);

	int getAge();               // các hàm dùng để lấy giá trị mong muốn của các thành phần private - đây là các hàm read-only
	float getMark();
	char* getName();
	char* getAddress();
	char* getID();
