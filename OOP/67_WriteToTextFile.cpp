...
int main() {
	...
	//ofs << message << endl;                       // cú pháp ghi vào file

	//ofs.close();

	Person p(20, "Tran Thu Thuy", "Ha Noi", "ngthuy@hmail.com");
	ofstream ofs2("output.pdf", ios::app);          // mode mở file: ios::out để ghi nội dung từ đầu cho toàn bộ file,  ios::app ghi tiếp nội dung cho file
	writeToFile(p, ofs2);
	ofs2.close();
