...
void Merge(int arr[], int l, int m, int r){
    ...
    for (u = 0; u < Left; u++)
        LeftArr[u] = arr[l+u];
    for (v = 0; v < Right; v++)
        RightArr[v] = arr[m+1+v];

    // vừa gộp 2 half lại vừa sắp xếp
    int i = 0, j = 0;
    int k = l;                                         //Lưu ý: xuất phát từ k = l (tổng quát) chứ không phải từ k = 0
...
void MergeSort(int arr[], int l, int r){
    if (l < r){                                     // Lưu ý phải có điều kiện này (rất quan trọng)
        int m = l + (r-l)/2;

        MergeSort(arr, l, m);
        MergeSort(arr, m + 1, r);                  // !!! QUAN TRỌNG NHẤT: m + 1 chứ không phải m 
